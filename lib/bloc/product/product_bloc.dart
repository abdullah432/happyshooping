...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  Stream<ProductState> mapEventToState(ProductEvent event) async* {
    if (event is FetchProduct) {
      ...
      try {
        listOfProduct =
            await _productRepository.loadProductsWithStoreId(storeID: event.storeID);
        //before displaying listOfProduct we should also know is that in user cart or not
        await _cartRepository.loadUserCart();
    ...
    if (event is AddToCart) {
      try {
        String result = await _cartRepository.addorRemoveCartListner(productID: event.productid);
        if (result != 'Success')
